...
locks="-l" #  [optional] print java.util.concurrent locks (BOOLEAN, false)
extended="-e" # [optional] print extended thread information (BOOLEAN, false)
options="${locks} ${extended}"

if $JCMD $PID Thread.print ${options} > "${thread_dump}"; then
  msg "Thread Dump successfully created: '${thread_dump}' for PID ${PID} (${pidname})."

else
  err "Failed to create Thread Dump: '${thread_dump}' to PID ${PID} (${pidname})."
fi
